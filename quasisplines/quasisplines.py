...
def tuple_to_SP(smg, in_tup):
   R = smg[_sage_const_0 ][_sage_const_0 ].parent()
   s = len(smg[_sage_const_0 ])
   srg = spline_ring_generators(smg)
   (SP, x, y) = SPxy(srg)
   one_tup = tuple([R(_sage_const_1 ) for i in range(s)])
   oi = None
   ...
   if oi == None:
      ...
      smg = [one_tup] + smg
   lx = list(x)
   lox = lx[:oi]+[SP(_sage_const_1 )]+lx[oi:]
   ox = tuple(lox)
...
def SP_to_tuple(SP, srg, f):
    sigma_lst = [sigma_SP_to_R(SP, srg, i) for i in range(len(srg[_sage_const_0 ]))]
...
def triangle_splines(triangle_list = [], r=_sage_const_0 ):
    r"""
    Returns a tuple of 
...
def expand_spline(g, smg):
   ...
   OA = expansion_coeffs[_sage_const_0 ].parent()
   s = len(g)
   zero_list = [_sage_const_0  for i in range(s)]
   OA_to_R = OA.hom(zero_list+list(R.gens()), R)
